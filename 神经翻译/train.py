...
Thirdly, divided data set into some batches.
    For each batches, padding training data such that its length equal to the longest sequence of this batch.
    What's more, padding training target until its length = max_length, which is a hyper parameter.
...
**********
What you should focus on is that the dimension of input is not batch first,
...
def padding_data(batch_data, num2word):
    """
    Padding training data until its length equal to the longest one in this batch
...
def padding_label(batch_data, num2word, length):
    """
    Padding training target until its length = the max length
    :param batch_data: a batch of data
    :param num2word: list, the dict number to word
    :param length: the max length of target
    :return: np.array, the data after padding
    """
    new_batch_data = list()
    for data in batch_data:
